...
class CreateGroubService extends Component
{
    public $GroupsItems = [];
    public $allServices = [];
    ...
    public function render()
    {
        ...
        return view('livewire.GroubService.create-groub-service', [
            'groups'=>Groub::all(),
    ...
    public function saveGroup()
    {
        ...
        // update
        if($this->updateMode){
            $Groups = Groub::find($this->group_id);
            $total = 0;
            foreach ($this->GroupsItems as $groupItem) {
                if ($groupItem['is_saved'] && $groupItem['service_price'] && $groupItem['quantity']) {
                    // الاجمالي قبل الخصم
    ...
    public function delete($id){
        Groub::destroy($id);
        return redirect()->route('Add_GroubServices');
    }
